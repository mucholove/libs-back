...
#ifndef HLSWindowBuffer_h
#define HLSWindowBuffer_h

#include <config.h>


struct HLSWindowBuffer_depth_info_s
{
...
/*
HLSWindowBuffer maintains an XImage for a window. Each ARTGState that
renders to that window uses the same HLSWindowBuffer (and thus the same
buffer, etc.).

Many states might render to the same window, so we need to make sure
that there's only one HLSWindowBuffer for each window. */
@interface HLSWindowBuffer : NSObject
{
@public
...
@private
  ...
  struct HLSWindowBuffer_depth_info_s DI;
...
/*
Returns a _retained_ HLSWindowBuffer for the specified gswindow_device_t.

The depth info is only used if a new HLSWindowBuffer needs to be allocated.
*/
