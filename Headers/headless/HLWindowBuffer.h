...
#ifndef HLWindowBuffer_h
#define HLWindowBuffer_h

#include <config.h>


struct HLWindowBuffer_depth_info_s
{
...
/*
HLWindowBuffer maintains an XImage for a window. Each ARTGState that
renders to that window uses the same HLWindowBuffer (and thus the same
buffer, etc.).

Many states might render to the same window, so we need to make sure
that there's only one HLWindowBuffer for each window. */
@interface HLWindowBuffer : NSObject
{
@public
...
@private
  ...
  struct HLWindowBuffer_depth_info_s DI;
...
/*
Returns a _retained_ HLWindowBuffer for the specified gswindow_device_t.

The depth info is only used if a new HLWindowBuffer needs to be allocated.
*/
+ (id)
    windowBufferForWindow: (gswindow_device_t *)awindow          
    depthInfo: (struct HLWindowBuffer_depth_info_s *)aDI;
